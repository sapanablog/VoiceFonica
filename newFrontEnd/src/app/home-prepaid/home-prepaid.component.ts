...
export class HomePrepaidComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    localStorage.setItem('act2','')
    localStorage.setItem('act3','')
  }

  load()
  {
    this.res=this._service.historyCustomerFromRemote(this.mb);
    this._service.getActivePlanFromRemote(this.mb).subscribe(data=>{
